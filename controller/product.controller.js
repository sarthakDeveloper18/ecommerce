    ...
    form.parse(req, (err, fields, files) => {
        ...
        if (files.photo) {
            if (files.photo.size > 1000000) {
                return res.status(400).json({ err: 'Image Size should be less than 1mb' });
            }
...
exports.relatedProduct = (req, res, next) => {
    let limit = req.query.limit ? parseInt(req.query.limit) : 6;
    Product.find({ _id: { $ne: req.product }, category: req.product.category }).limit(limit).populate('category', '_id name').exec((err, data) => {
        if (err) return res.status(400).json({ err: 'No Product Related Found' });
        res.send(data);
    })
}
...
exports.listCategories = (req, res, next) => {
    Product.distinct('category', {}, (err, data) => {
        if (err) return res.status(400).json({ err: 'No List Categories Found' });
        res.json(data);
    })
}
...
exports.decreaseQuantity = (req, res, next) => {
    ...
    Product.bulkWrite(bulk, {}, (err, data) => {
        if (err) return res.status(400).json({
